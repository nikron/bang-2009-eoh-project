...
static int listen_new_peers() {
	...
	if (strncmp(buf, "BANG?", SCAN_MSG_LEN) == 0) {
		//Answer it
		if (send_bang_reply(inet_ntop(new_peer_addr.ss_family, get_in_addr((struct sockaddr *)&new_peer_addr), s, sizeof s)) != 0)
			return 1;
		...
		//Otherwise if it was an answer to our call
			...
			scanArg.length = sizeof(struct sockaddr_storage);
